  ...
  return (
    <SidebarContext.Provider value={contextValue}>
      <div
        ...
        className={cn(
          // Local change from the registry: `min-h-svh` -> `relative h-full min-h-0`.
          // This app has a header, a recovery banner and a compact bar above the
          // workspace, so the provider is a row inside that column rather than the
          // page itself. `relative` is what the desktop panel below anchors to.
  ...
  return (
    ...
    >
      ...
      <div
        ...
        data-side={side}
        className={cn(
          // Local change from the registry: `fixed`/`h-svh` -> `absolute`/`h-full`,
          // so the panel spans the workspace row instead of the whole viewport.
          // Left fixed it slides under the app header. See the provider above.
